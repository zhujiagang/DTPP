...
def rgb_stack_oversample(flow_stack, crop_dims):
    """
...
def oversample_for_rgb_stack(images, crop_dims, stack_len = 25):
    """
    Crop images into the four corners, center, and their mirrored versions.
    ...
    images = np.tile(images, (2, 1, 1, 1))
    ix = 0
    for crop in crops_ix:
        for im in images:
            crops[ix] = im[crop[0]:crop[2], crop[1]:crop[3], :]
            ix += 1
        crops[ix-stack_len:ix] = crops[ix-stack_len:ix, :, ::-1, :]  # flip for mirrors
    return crops
...
def flow_stack_oversample_new(flow_stack, crop_dims):
    ...
    # cropsx = np.empty((250, 10, crop_dims[0], crop_dims[1]), dtype=flow_stack.dtype)
    #
    # for iii in range(10):
    #     cropsx[25*iii : 25*(iii+1)] = crops[iii].reshape(25,10,224,224)

    return crops

def oversample_for_flow_stack_test(images, crop_dims, stack_len = 25):
    # images = np.array(images).reshape(25,256,340,10)
    ...
    for crop in crops_ix:
        for im in images:
            ...
            ix += 1
        crops[ix-stack_len:ix] = crops[ix-stack_len:ix, :, ::-1, :]  # flip for mirrors
        crops[ix-stack_len:ix, :,:, range(0, 10, 2)] = 255 - crops[ix-stack_len:ix, :,:, range(0, 10, 2)]
